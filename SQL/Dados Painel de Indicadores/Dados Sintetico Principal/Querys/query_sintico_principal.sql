SELECT CD_PRESTADOR
      ,nm_prestador
      ,max(nota_tempo_casa) NOTA_TEMPO_CASA
      ,sum(NOTA_URGENCIA) NOTA_URGENCIA
      ,sum(NOTA_EXAME) NOTA_EXAME
      ,sum(NOTA_ELETIVOS) NOTA_ELETIVOS
      ,sum(NOTA_CIRURGICO) NOTA_CIRURGICO
      ,sum(NOTA_INCONFORMIDADE) NOTA_INCONFORMIDADE
      ,sum(NOTA_GLOSA) NOTA_GLOSA
      ,max(NOTA_CAPACITACAO) NOTA_CAPACITACAO
      ,min(NOTA_EVE_ADVERSO) NOTA_EVE_ADVERSO
      ,sum(NOTA_INFECCAO) NOTA_INFECCAO
      ,round(sum(NOTA_REINTERVENCAO) / 12) NOTA_REINTERVENCAO
      ,sum(NOTA_SAT_CLI_EXT) NOTA_SAT_CLI_EXT
      ,sum(NOTA_TICKET_MEDIO) NOTA_TICKET_MEDIO
      ,(max(nota_tempo_casa) + sum(NOTA_URGENCIA) + sum(NOTA_EXAME) +
       sum(NOTA_ELETIVOS) + sum(NOTA_CIRURGICO) +
       sum(NOTA_INCONFORMIDADE) + sum(NOTA_GLOSA) + max(NOTA_CAPACITACAO) +
       max(NOTA_EVE_ADVERSO) + sum(NOTA_INFECCAO) + sum(NOTA_SAT_CLI_EXT) +
       sum(NOTA_TICKET_MEDIO)) Total
      ,DENSE_RANK() OVER(ORDER BY(max(nota_tempo_casa) + sum(NOTA_URGENCIA) + sum(NOTA_EXAME) + sum(NOTA_ELETIVOS) + sum(NOTA_CIRURGICO) + sum(NOTA_INCONFORMIDADE) + sum(NOTA_GLOSA) + max(NOTA_CAPACITACAO) + max(NOTA_EVE_ADVERSO) + sum(NOTA_INFECCAO) + sum(NOTA_SAT_CLI_EXT) + sum(NOTA_TICKET_MEDIO)) DESC) AS Ranking
  FROM (
